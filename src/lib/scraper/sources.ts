...
// Sources:
// 1. darkweb — unified dark-web source: Tor-based dark-web search (6 engines) + RansomLook leak-site monitoring. AI-filtered for automotive only.
...
export const SOURCE_DEFS: SourceDef[] = [
 ...
 description: "Dark-web intelligence: Tor-based dark-web search (6 .onion engines) + RansomLook leak-site monitoring with health checks. AI-filtered for automotive relevance only. Requires Tor for .onion scraping.",
 isDarkWeb: true,
 },
 {
...
// 1) darkweb — unified dark-web source (RansomLook + Tor-based dark-web search).
...
// b. Tor-based: search 6 .onion engines via Tor → scrape → AI classify (only if Tor available)
...
export async function fetchDarkweb(): Promise<RawItem[]> {
 ...
 // RansomLook may be unavailable; continue to pipeline.
 }

 // ── Sub-pipeline B: Tor-based dark-web search ───────────────────────────────
