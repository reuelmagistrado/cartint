...
import {
  Grid3x3, Filter, Brain, Loader2, Target, Eye, AlertCircle, GitBranch,
  ChevronDown, ChevronUp, ExternalLink, Tag,
} from "lucide-react";
import type { AtmTacticData, Threat, AtmMappingResult } from "./types";

// Distinct accent colors for each tactic (cycling palette — no indigo/blue)
const TACTIC_COLORS = [
  "#10b981", // emerald
  "#f59e0b", // amber
  "#f43f5e", // rose
...
export function AtmMatrixView({
  ...
  const [selectedTech, setSelectedTech] = useState<{ id: string; name: string; description: string; tacticName: string; tacticId: string } | null>(null);
  ...
  useEffect(() => {
    if (!selectedThreat) { setMapping(null); return; }
  ...
  const highlightedTactics = useMemo(() => {
    ...
    if (mapping) {
      ...
      for (const t of mapping.tactics) for (const tech of t.techniques) techniqueNames.add(tech.name.toLowerCase());
  ...
  // Build a map of technique → all tactics it appears in (for the detail modal)
  const techniqueTacticsMap = useMemo(() => {
    const map = new Map<string, { tacticName: string; tacticId: string }[]>();
    for (const tactic of tactics) {
      for (const tech of tactic.techniques) {
        if (!map.has(tech.id)) map.set(tech.id, []);
        map.get(tech.id)!.push({ tacticName: tactic.name, tacticId: tactic.tacticId });
      }
    }
    return map;
  }, [tactics]);
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="text-[11px] text-slate-400">
              {tactics.length} tactics · {tactics.reduce((s, t) => s + t.techniques.length, 0)} technique instances · horizontally scrollable
            </p>
          </div>
          <div className="flex items-center gap-2">
            ...
            <Select
              ...
              onValueChange={(v) => { setSelectedThreatId(v); setMapping(null); setShowMappingDetails(false); }}
            >
                ...
                {threats.map((t) => (
                  <SelectItem key={t.id} value={t.id}>
                    <span className="flex items-center gap-1.5">
                      <span className={`h-1.5 w-1.5 rounded-full ${t.atmTactic ? "bg-emerald-500" : "bg-slate-600"}`} />
          ...
          <div className="mt-2">
            {mappingLoading ? (
                ...
                <div className="flex items-center justify-between gap-2 rounded-lg border border-emerald-500/30 bg-emerald-500/5 px-3 py-2">
                  <div className="flex flex-wrap items-center gap-2 text-[11px] text-emerald-300">
                    ...
                    {mapping.tactics.map((t, i) => (
                      <Badge key={i} variant="outline" className="h-4 border-emerald-500/40 bg-emerald-500/10 px-1 text-[9px] text-emerald-300">{t.name}</Badge>
                    ))}
                  ...
                  <Button size="sm" variant="ghost" className="h-6 px-2 text-[10px] text-slate-400 hover:bg-slate-800 hover:text-slate-200" onClick={() => setShowMappingDetails(!showMappingDetails)}>
                    {showMappingDetails ? <ChevronUp className="h-3 w-3" /> : <ChevronDown className="h-3 w-3" />}
                    {showMappingDetails ? "Hide" : "Show"} analysis
                  </Button>
                </div>
                {showMappingDetails && <MappingDetails mapping={mapping} onRemap={() => runMapping(selectedThreat.id)} mappingLoading={mappingLoading} />}
              </div>
              ...
              <div className="rounded-lg border border-emerald-500/30 bg-emerald-500/5 px-3 py-1.5 text-[11px] text-emerald-300">
                ...
                <button onClick={() => runMapping(selectedThreat.id)} className="ml-2 text-cyan-400 underline hover:text-cyan-300">Re-map with AI</button>
              </div>
            ) : null}
          </div>
        )}
      ...
      <div className="overflow-x-auto pb-3">
        <div className="flex min-w-max gap-3">
          {tactics.map((tactic, i) => {
            const accentColor = TACTIC_COLORS[i % TACTIC_COLORS.length];
            ...
            return (
              <motion.div
                ...
                className={`flex w-[200px] shrink-0 flex-col overflow-hidden rounded-xl border ${
                  isPrimary ? "border-emerald-500/60 shadow-lg shadow-emerald-500/10" : isHighlighted ? "border-cyan-500/40" : "border-slate-700/60"
                } bg-slate-900/40`}
              >
                {/* Tactic header — colored bar */}
                <div className="shrink-0" style={{ borderTop: `3px solid ${accentColor}` }}>
                  <div className="px-3 py-2.5">
                    <div className="flex items-center justify-between gap-1.5">
                      <span className="font-mono text-[9px] text-slate-500">{tactic.tacticId}</span>
                      <span className="rounded px-1.5 py-0.5 font-mono text-[9px] font-bold" style={{ color: accentColor, backgroundColor: `${accentColor}15` }}>
                        {tactic.techniques.length}
                      </span>
                    </div>
                    <h3 className="mt-1 text-xs font-bold leading-tight text-slate-100">{tactic.name}</h3>
                  </div>
                </div>

                {/* Technique cards */}
                <div className="flex-1 space-y-1 overflow-y-auto p-1.5" style={{ maxHeight: "calc(100vh - 280px)" }}>
                  {tactic.techniques.map((tech) => {
                    ...
                    return (
                      <button
                        ...
                        onClick={() => setSelectedTech({ ...tech, tacticName: tactic.name, tacticId: tactic.tacticId })}
                        className={`block w-full rounded-md border p-1.5 text-left transition-all hover:border-slate-500 ${
                          isTechHighlighted
                      ...
                      >
                        ...
                        <p className={`mt-0.5 text-[10px] font-medium leading-tight ${isTechHighlighted ? "text-emerald-100" : "text-slate-200"}`}>
                          {tech.name}
                        </p>
                      </button>
                    );
      ...
      <Dialog open={!!selectedTech} onOpenChange={(v) => !v && setSelectedTech(null)}>
        <DialogContent className="max-w-lg border-slate-700 bg-slate-950 p-0">
          {selectedTech && (
            <>
              <DialogHeader className="border-b border-slate-800 p-5 pb-4">
                <div className="flex items-center gap-2">
                  <span className="rounded bg-slate-800 px-2 py-0.5 font-mono text-[10px] font-bold text-emerald-300">{selectedTech.id}</span>
                  <DialogTitle className="text-base font-semibold text-slate-50">{selectedTech.name}</DialogTitle>
                </div>
              </DialogHeader>
              <div className="space-y-4 p-5">
                ...
                <div>
                  <p className="mb-1.5 flex items-center gap-1 text-[10px] font-semibold uppercase tracking-wider text-slate-400">
                    <Tag className="h-3 w-3" /> Appears in Tactics
                  </p>
                  <div className="flex flex-wrap gap-1.5">
                    {(techniqueTacticsMap.get(selectedTech.id) || []).map((t, i) => (
                      <Badge
                        key={i}
                        variant="outline"
                        className={`h-5 border px-1.5 text-[10px] ${
                          t.tacticName === selectedTech.tacticName
                            ? "border-emerald-500/40 bg-emerald-500/10 text-emerald-300"
                            : "border-slate-600 bg-slate-800/40 text-slate-300"
                        }`}
                      >
                        <span className="mr-1 font-mono text-[8px] text-slate-500">{t.tacticId}</span>
                        {t.tacticName}
                      </Badge>
                    ))}
                  </div>
                ...
                <div>
                  ...
                  <p className="text-sm leading-relaxed text-slate-300">{selectedTech.description}</p>
                </div>

                {/* Reference link */}
                <div className="border-t border-slate-800 pt-3">
                  <p className="mb-1.5 text-[10px] font-semibold uppercase tracking-wider text-slate-400">Reference</p>
                  <a
                    href={`https://atm.automotiveisac.com/technique/${selectedTech.tacticId}/${selectedTech.id}`}
                    target="_blank"
                  ...
                  >
                    ...
                    Auto-ISAC ATM Resource
                    <span className="font-mono text-[9px] text-slate-500">atm.automotiveisac.com</span>
                  </a>
                </div>
              </div>
            </>
...
// ─── AI Mapping Details (expandable) ───────────────────────────────────────

function MappingDetails({ mapping, onRemap, mappingLoading }: { mapping: AtmMappingResult; onRemap: () => void; mappingLoading: boolean }) {
  return (
    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} className="space-y-3 rounded-lg border border-cyan-500/20 bg-slate-900/50 p-3">
      ...
      <div>
        <p className="mb-1.5 text-[10px] font-semibold uppercase tracking-wider text-slate-400">Mapped Tactics & Techniques ({mapping.tactics.length})</p>
        <div className="space-y-1.5">
          {mapping.tactics.map((t, ti) => (
            <div key={ti} className="rounded border border-slate-700/40 bg-slate-800/20 p-2">
              <div className="flex items-center gap-1.5">
                <Badge variant="outline" className={`h-4 border px-1 text-[9px] ${t.evidenceType === "observed" ? "border-emerald-500/40 bg-emerald-500/10 text-emerald-300" : "border-amber-500/40 bg-amber-500/10 text-amber-300"}`}>{t.evidenceType}</Badge>
                ...
                <div className="mt-1.5 space-y-1">
                  {t.techniques.map((tech, thi) => (
                    <div key={thi} className="flex items-start gap-1.5 rounded bg-slate-900/40 px-1.5 py-1">
                      <Badge variant="outline" className="h-3.5 shrink-0 border-slate-600 px-1 font-mono text-[8px] text-slate-400">{tech.id}</Badge>
                      <Badge variant="outline" className={`h-3.5 shrink-0 border px-1 text-[8px] ${tech.evidenceType === "observed" ? "border-emerald-500/30 bg-emerald-500/10 text-emerald-300" : "border-amber-500/30 bg-amber-500/10 text-amber-300"}`}>{tech.evidenceType}</Badge>
      ...
      <div className="flex items-center justify-between border-t border-slate-700/40 pt-2">
        ...
        <Button size="sm" variant="outline" onClick={onRemap} disabled={mappingLoading} className="h-6 border-cyan-500/40 bg-cyan-500/10 text-cyan-200 hover:bg-cyan-500/20">
          {mappingLoading ? <Loader2 className="h-3 w-3 animate-spin" /> : <Brain className="h-3 w-3" />} Re-map
        </Button>
      </div>
    </motion.div>
  );
...
function ContextRow({ icon: Icon, label, value }: { icon: React.ElementType; label: string; value: string }) {
  return (
    <div className="min-w-0">
      <p className="flex items-center gap-1 text-[10px] uppercase tracking-wider text-slate-500"><Icon className="h-3 w-3" /> {label}</p>
  ...
  return (
    <div className="min-w-0">
      ...
      <ul className="mt-0.5 space-y-0.5">
        {items.slice(0, 5).map((item, i) => (<li key={i} className="text-[10px] leading-tight text-slate-400">{item}</li>))}
